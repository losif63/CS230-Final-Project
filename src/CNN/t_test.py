...
Created on Dec 3, 2025
    ...
    Part of CS230 final project - module created to compare different performances across models when different inputs are given.
    
    The model results are collected and hard-coded in the module.

@author: Sebastian Prepelita
'''
import numpy as np

cnn_1d_euclidian = [0.0810 , 0.0838 , 0.0737 , 0.1268 , 0.0716 , 0.0773 , 0.0737 , 0.0670 ,  0.0963 , 0.1084 , 0.0727 , 0.0654 , 
                    0.0762 , 0.1471 , 0.0845 , 0.1194 , 0.0762 , 0.1136 , 0.1147 , 0.1210 , 0.0830 , 0.0955 , 0.1345,
                    1.1646 # weight decay one
                    ]
cnn_1d_rotation = [14.21, 12.98, 14.25, 13.14, 15.08, 14.8 , 13.67, 15.11, 19.42, 14.58, 13.2 , 12.56, 13.6 , 16.24, 13.43, 14.6 , 16.19, 16.23, 14.45, 16.65, 13.72, 13.65, 14.35,
                   23.55 # weight decay one
                   ]

cnn_2d_gd_euclidian = [0.107785, 0.079158, 0.076254, 0.109989, 0.100035, 0.102524, 0.075228, 0.084049, 0.097427, 0.087603, 0.076512, 0.089653, 0.07898 , 0.106473, 0.108956, 0.093121, 0.086473, 0.094593, 0.070833, 0.073481, 0.094443, 0.078956, 0.069395, 0.083236, 0.070075]
...
if __name__ == '__main__':
    ...
    print("= Position")
    print("="*60)
    print(f"average 1D: {np.round(np.average(cnn_1d_euclidian),4)} ({np.round(np.std(cnn_1d_euclidian,ddof=1),4)})")
    print(f"average 2D GD: {np.round(np.average(cnn_2d_gd_euclidian),4)} ({np.round(np.std(cnn_2d_gd_euclidian,ddof=1),4)})")
    print(f"average 2D spectogram: {np.round(np.average(cnn_2d_spectogram_euclidian),4)} ({np.round(np.std(cnn_2d_spectogram_euclidian,ddof=1),4)})")
    # Perform the independent two-sample t-test (equal_var=True by default)
    t_statistic, p_value = stats.ttest_ind(cnn_1d_euclidian, cnn_2d_gd_euclidian, equal_var=False)
    ...
    print(f"\t2D gd vs 2D spectogram, p-value: {np.round(p_value,8)}")
    print("="*60)
