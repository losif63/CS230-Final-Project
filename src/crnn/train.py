...
from crnn_model import CRNNRotation
...
def train_model(model: torch.nn.Module,
    ...
    for epoch in range(num_epochs):
        ...
        if val_metrics['angular_error_deg'] < best_angular_error:
            best_angular_error = val_metrics['angular_error_deg']
            best_val_loss = val_metrics['loss']
            checkpoint_path = os.path.join(save_dir, 'best_crnn_model.pth')
            torch.save({
                'epoch': epoch,
                'model_state_dict': model.state_dict(),
                'optimizer_state_dict': optimizer.state_dict(),
                'angular_error_deg': best_angular_error,
                'val_loss': best_val_loss,
                'val_metrics': val_metrics,
                'history': history
            }, checkpoint_path)
            print(f" Saved best model (angular error: {best_angular_error:.2f})")
